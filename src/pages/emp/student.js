...
const Student = () => {
  ...
  const Details = ({student})=>{
    const changeIt = async () => {
      try {
        ...
        const res = await axios.put('https://placementor.onrender.com/emp/updatestudent/' + emp.id , {
          name: student.name,
          email: student.email
        },{
          headers: { 
  ...
  const getData = async () => {
    const emp = JSON.parse(localStorage.getItem('emp'));
    const res = await axios.get('https://placementor.onrender.com/emp/getstudent/' + emp.id , {
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + emp.token,
      }
  ...
  const AddStudent = () => {
    ...
    const onSubmit = async (data) => {
      try {
        const emp = JSON.parse(localStorage.getItem('emp'));
        const res = await axios.post('https://placementor.onrender.com/emp/addstudent/' + emp.id, data, {
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${emp.token}`
          }
