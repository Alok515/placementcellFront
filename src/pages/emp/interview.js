...
const Interview = () => {
  ...
  const Details = ({interview}) => {
    ...
    const addItHere = async () => {
      ...
      const res = await axios.post('https://placementor.onrender.com/emp/updateinterview/' + emp.id ,{
        id: interview._id,
        studentId,
      }, {
        headers: { 
  ...
  const getData = async () => {
    const emp = JSON.parse(localStorage.getItem('emp'));
    const res = await axios.get('https://placementor.onrender.com/emp/getinterview/' + emp.id , {
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + emp.token,
      }
    ...
    if (res.status === 200) {
      ...
      const studentFetch = await axios.get('https://placementor.onrender.com/emp/getstudent/' + emp.id , {
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + emp.token,
        }
  ...
  const AddInterview = () => {
    ...
    const onSubmit = async (data) => {
      try {
        const emp = JSON.parse(localStorage.getItem('emp'));
        const res = await axios.post('https://placementor.onrender.com/emp/addinterview/' + emp.id, data, {
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${emp.token}`
          }
